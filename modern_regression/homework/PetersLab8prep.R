...
# The support vectors are crosses
#Other observations are circles

svmfit$index
# index for support vectors
...
#Try using small cost:

svmfit = svm(y ~ ., data = dat , kernel = "linear",cost = 0.1, scale = FALSE)
plot(svmfit , dat)
#more support vectors because of the lower cost

svmfit$index

#Provide the range of cost values to figure out the width 
...
#find the cross-validation errors
summary(tune.out)

#look at the lowest cost
...
#Predict class labels of the data set through the test observations
...
#separate them more when plotting
x[y == 1, ] = x[y == 1, ] + 0.5
plot(x, col = (y + 5) / 2, pch = 19)

# use high cost for little misclassification of vectors
dat = data.frame(x = x, y = as.factor(y))
...
# only 3 support vectors and no misclassifications
...
# It seems that it will perform better than cost of 5 on the testing data
...
summary(tune.out)
# the best choice of parameters is the second line or the one with
# cost = 1 and gamma = .5

table(true = dat[-train , "y"],pred = predict(tune.out$best.model, 
                                              newdata = dat[-train , ]))

# 12/100 = 12% of test observations are misclassified
...
#nstart >1 then K-means clustering will be performed using multiple random 
#assignments
...
# downside: for real data we don't know how many clusters
...
#Complete and average linkage separate observations into correct group. 
#Single linkage identifies one point as belonging own cluster.

cutree(hc.single,4)
#much more reasonable tree, but still two single points
...
#Correlation-based distance computed using the as.dist() function.
